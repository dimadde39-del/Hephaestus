    ...
    def deliberate(
        ...
        passes = [
            ...
            DeliberationPass(
                ...
                purpose="Bring relevant durable memories into the turn.",
                findings=_memory_findings(context),
                confidence=0.72 if context.memories else 0.58,
            ),
            DeliberationPass(
...
def _memory_findings(context: RetrievedConversationContext) -> list[str]:
    ...
    return [memory.summary or memory.content for memory in context.memories[:5]]
...
def _next_moves(
    ...
    if mode == DeliberationMode.RESEARCH:
        return [
            "List the claim that needs evidence.",
            "Identify primary sources or direct comparisons.",
...
def _synthesize_deterministic_response(
    ...
    *,
    ...
    if context.memories:
        memory_line = "\n\nSelected memory:\n" + "\n".join(
            f"- {memory.summary or memory.content}" for memory in context.memories[:4]
        )
